...
def appveyor_vars():
    ...
    message_extended = environ.get('CIRRUS_CHANGE_MESSAGE')
    configuration_name = environ.get('CIRRUS_TASK_NAME')
    branch = environ.get('CIRRUS_BRANCH')
    author = environ.get('')
    author_email = environ.get('')
    timestamp = environ.get('')
    repo_provider = 'github'
    project_name = environ.get('CIRRUS_REPO_NAME')
    project_slug = environ.get('')
    pull_request_title = environ.get('CIRRUS_CHANGE_TITLE')
    build_version = environ.get('CIRRUS_TASK_ID')
    commit = environ.get('CIRRUS_CHANGE_IN_REPO')
    message = environ.get('CIRRUS_CHANGE_MESSAGE')
    account_name = environ.get('')
    pull_request_number = environ.get('CIRRUS_PR')
    repo_name = environ.get('CIRRUS_REPO_FULL_NAME')

    short_commit = commit[:12]
    build_url = 'https://cirrus-ci.com/build/{build_version}'.format(**locals())
