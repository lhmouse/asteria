...
# setup
export CXX=${CXX:-"g++"}
export CPPFLAGS="-D_FILE_OFFSET_BITS=64 -D_POSIX_C_SOURCE=200809 -D_GNU_SOURCE"
export CXXFLAGS='-std=c++17 -fno-gnu-keywords -Wno-zero-as-null-pointer-constant'

# note: `sem` is not always available
_sem="parallel --will-cite --semaphore --halt soon,fail=1"

for _file in $(find -L "src" -name "*.[hc]pp")
do
