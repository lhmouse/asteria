      ...
      case index_array_random:
        {
          ...
          size_t off = probe_origin(arr.size(), altr.seed);
          return arr.data() + off;
        }
      ...
      case index_array_index:
        {
          ...
          if(w.nprepend)
            arr.insert(arr.begin(), clamp_cast<size_t>(w.nprepend, 0U, SIZE_MAX));
          else if(w.nappend)
            arr.append(clamp_cast<size_t>(w.nappend, 0U, SIZE_MAX));
