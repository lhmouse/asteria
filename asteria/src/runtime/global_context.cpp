...
constexpr s_modules[] =
  {
    ...
    { api_version_0002_0000,  "rsa",         create_bindings_rsa         },
  };
...
Global_Context(API_Version api_version_req)
  :
    m_gcoll(make_refcnt<Garbage_Collector>()),
    m_prng(make_refcnt<Random_Engine>()),
    m_ldrlk(make_refcnt<Module_Loader>())
  {
...
#ifdef ASTERIA_DEBUG
    ASTERIA_ASSERT(::std::is_sorted(begin(s_modules), end(s_modules), comp));
#endif
    ...
    ::std::for_each(bptr, eptr,
      [&](const Module& mod) {
        ...
        mod.init(r.first->second.open_object(), eptr[-1].api_version);
      });
