...
do_create_lazy_reference_opt(Reference* hint_opt, const phcow_string& name)
  const
  {
    ...
    if(name == "__this") {
      ...
      ASTERIA_ASSERT(ref.is_invalid());
      return &ref;
    }

    if(name == "__func") {
      ASTERIA_ASSERT(this->m_func);
      auto& ref = this->do_mut_named_reference(hint_opt, name);
    ...
    if(name == "__varg") {
      ASTERIA_ASSERT(this->m_func);
      auto& ref = this->do_mut_named_reference(hint_opt, name);
      auto varg = make_refcnt<Variadic_Arguer>(*(this->m_func), this->m_lazy_args);
...
do_on_scope_exit_normal_slow()
  {
    ...
    if(saved_status == air_status_return) {
      ...
      ASTERIA_ASSERT(!saved_result.is_invalid());
    }
    ...
    while(!this->m_defer.empty()) {
      ...
      try {
        ...
        ASTERIA_ASSERT(*(this->m_status) == air_status_next);
      }
