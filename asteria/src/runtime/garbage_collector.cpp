...
do_collect_generation(uint32_t gen)
  {
    ...
    this->m_recur ++;
    const unique_ptr<int, void (int*)> recur_guard(
    ...
    while(this->m_temp_1.extract_variable(var)) {
      ...
      // initialized to one.
      var->m_gc_ref = 1;
      ASTERIA_ASSERT(var->m_gc_ref <= var->use_count() - 1);
    ...
    while(this->m_temp_1.extract_variable(var)) {
      ...
      if(ASTERIA_EXPECT(var->m_gc_ref == var->use_count() - 1))
