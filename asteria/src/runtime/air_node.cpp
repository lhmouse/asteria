...
#include "../../runtime/module_loader.hpp"
...
do_evaluate_subexpression(Executive_Context& ctx, bool assign, const AVM_Rod& rod)
  {
    if(rod.empty()) {
      ...
      ASTERIA_ASSERT(ctx.status() == air_status_next);
    }
    else if(assign) {
...
do_invoke_partial(Reference& self, Executive_Context& ctx, const Source_Location& sloc,
                  PTC_Aware ptc, const cow_function& target)
  {
    ...
    if(ASTERIA_EXPECT(ptc == ptc_aware_none)) {
      ...
      // Perform a tail call.
      self.set_ptc(make_refcnt<PTC_Arguments>(sloc, ptc,
          ...
          rod.push_function(
            +[](Executive_Context& ctx, const AVM_Rod::Header* head)
              __attribute__((__hot__, __flatten__))
              {
                ...
                ASTERIA_ASSERT(var && !var->initialized());
              ...
              __attribute__((__cold__))
              {
                ...
                for(size_t k = 0;  k < sp.size();  ++k)
                  if(sp.at(k).type == switch_clause_default) {
                    ...
                    if(is_none_of(cmp_lo, { compare_greater, sp.at(k).cmp2_lower })
                        || is_none_of(cmp_up, { compare_less, sp.at(k).cmp2_upper }))
                      continue;
                  ...
                  for(size_t i = 0;  i < sp.size();  ++i)
                    if(i < target_index) {
                      ...
                      if(ctx.status() != air_status_next) {
                        if(is_any_of(ctx.status(), { air_status_break, air_status_break_switch }))
          ...
          rod.push_function(
            +[](Executive_Context& ctx, const AVM_Rod::Header* head)
              __attribute__((__hot__, __flatten__))
              {
                ...
                for(;;) {
                  do_execute_block(sp.rod_body, ctx);
                  if(is_any_of(ctx.status(), { air_status_continue, air_status_continue_while }))
                    ctx.status() = air_status_next;
                  else if(ctx.status() != air_status_next) {
                    if(is_any_of(ctx.status(), { air_status_break, air_status_break_while }))
                      ctx.status() = air_status_next;
                    break;
                  }
            ...
            +[](Executive_Context& ctx, const AVM_Rod::Header* head)
              __attribute__((__hot__, __flatten__))
              {
                ...
                try {
                  ...
                  ASTERIA_ASSERT(ctx_for.status() == air_status_next);
                  ...
                  *mapped_ref = move(ctx_for.stack().mut_top());
                  ...
                  if(range.is_array()) {
                    ...
                    for(;;) {
                      ...
                      if(is_any_of(ctx.status(), { air_status_continue, air_status_continue_for }))
                        ctx.status() = air_status_next;
                      else if(ctx.status() != air_status_next) {
                        if(is_any_of(ctx.status(), { air_status_break, air_status_break_for }))
                          ctx.status() = air_status_next;
                        break;
                      }
                    }
                ...
                try {
                  ...
                  try {
                    ...
                    for(size_t k = 0;  k < except.count_frames();  ++k) {
                      V_object r;
                      r.try_emplace(&"frame", sref(describe_frame_type(except.frame(k).type)));
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(rhs.is_integer()) {
                      ...
                      bool ovr = false;
                      V_integer result = addm(val, 1, &ovr);
                      if(ovr)
                        throw Runtime_Error(xtc_format,
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(rhs.is_integer()) {
                      ...
                      V_integer& val = rhs.open_integer();
                      bool ovr = false;
                      V_integer result = subm(val, 1, &ovr);
                      if(ovr)
                        throw Runtime_Error(xtc_format,
                           "Integer increment overflow (operand was `$1`)", val);
                      if(postfix)
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(rhs.is_integer()) {
                      ...
                      V_integer& val = rhs.open_integer();
                      bool ovr = false;
                      V_integer result = subm(0, val, &ovr);
                      if(ovr)
                        throw Runtime_Error(xtc_format,
              ...
              rod.push_function(
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    // typeof
                    rhs = V_string(sref(describe_type(rhs.type())));
                  }
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(rhs.is_integer()) {
                      // __lzcnt integer
                      V_integer& val = rhs.open_integer();
                      val = static_cast<V_integer>(lzcnt64(static_cast<uint64_t>(val)));
                    }
                    else
                      throw Runtime_Error(xtc_format,
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(rhs.is_integer()) {
                      // __tzcnt integer
                      V_integer& val = rhs.open_integer();
                      val = static_cast<V_integer>(tzcnt64(static_cast<uint64_t>(val)));
                    }
                    else
                      throw Runtime_Error(xtc_format,
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(rhs.is_integer()) {
                      // __popcnt integer
                      V_integer& val = rhs.open_integer();
                      val = static_cast<V_integer>(popcnt64(static_cast<uint64_t>(val)));
                    }
                    else
                      throw Runtime_Error(xtc_format,
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(lhs.is_integer() && rhs.is_integer()) {
                      ...
                      bool ovr = false;
                      V_integer result = addm(val, rhs.as_integer(), &ovr);
                      if(ovr)
                        throw Runtime_Error(xtc_format,
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(lhs.is_integer() && rhs.is_integer()) {
                      ...
                      V_integer& val = lhs.open_integer();
                      bool ovr = false;
                      V_integer result = subm(val, rhs.as_integer(), &ovr);
                      if(ovr)
                        throw Runtime_Error(xtc_format,
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(lhs.is_integer() && rhs.is_integer()) {
                      ...
                      V_integer& val = lhs.open_integer();
                      bool ovr = false;
                      V_integer result = mulm(val, rhs.as_integer(), &ovr);
                      if(ovr)
                        throw Runtime_Error(xtc_format,
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(lhs.is_integer() && rhs.is_integer()) {
                      // __addm integer
                      V_integer& val = lhs.open_integer();
                      V_integer result = addm(val, rhs.as_integer());
                      val = result;
                    }
                    else
                      throw Runtime_Error(xtc_format,
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(lhs.is_integer() && rhs.is_integer()) {
                      // __subm integer
                      V_integer& val = lhs.open_integer();
                      V_integer result = subm(val, rhs.as_integer());
                      val = result;
                    }
                    else
                      throw Runtime_Error(xtc_format,
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(lhs.is_integer() && rhs.is_integer()) {
                      // __mulm integer
                      V_integer& val = lhs.open_integer();
                      V_integer result = mulm(val, rhs.as_integer());
                      val = result;
                    }
                    else
                      throw Runtime_Error(xtc_format,
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(lhs.is_integer()) {
                      ...
                      V_integer& val = lhs.open_integer();
                      int64_t n = min(rhs.as_integer(), 63);
                      ...
                      V_string& val = lhs.open_string();
                      size_t n = clamp_cast<size_t>(rhs.as_integer(), 0, val.ssize());
                      val.erase(0, n);
                      val.append(n, '\0');
                    }
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(lhs.is_integer()) {
                      ...
                      V_string& val = lhs.open_string();
                      size_t n = clamp_cast<size_t>(rhs.as_integer(), 0, PTRDIFF_MAX);
                      val.append(n, '\0');
                    }
                    else if(lhs.is_array()) {
            ...
            +[](Executive_Context& ctx, const AVM_Rod::Header* head)
              __attribute__((__cold__))
              {
                ...
                for(uint32_t i = nelems - 1;  i != UINT32_MAX;  --i) {
                  ...
                  if(ASTERIA_EXPECT(init.is_array()))
                    if(auto ielem = init.as_array().ptr(i))
                      var->initialize(*ielem);

                  if(ASTERIA_UNEXPECT(!var->initialized()))
            ...
            +[](Executive_Context& ctx, const AVM_Rod::Header* head)
              __attribute__((__cold__))
              {
                ...
                for(auto it = sp.keys.rbegin();  it != sp.keys.rend();  ++it) {
                  ...
                  if(ASTERIA_EXPECT(init.is_object()))
              ...
              __attribute__((__cold__))
              {
                ...
                if(abs_path[0] != '/') {
                  ...
                  if(src_file[0] == '/') {
                    ...
                    ASTERIA_ASSERT(pos != cow_string::npos);
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(lhs.is_integer()) {
                      ...
                      V_integer& val = lhs.open_integer();
                      bool ovr = false;
                      V_integer result = addm(val, irhs, &ovr);
                      if(ovr)
                        throw Runtime_Error(xtc_format,
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(lhs.is_integer()) {
                      ...
                      V_integer& val = lhs.open_integer();
                      bool ovr = false;
                      V_integer result = subm(val, irhs, &ovr);
                      if(ovr)
                        throw Runtime_Error(xtc_format,
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(lhs.is_integer()) {
                      ...
                      V_integer& val = lhs.open_integer();
                      bool ovr = false;
                      V_integer result = mulm(val, irhs, &ovr);
                      if(ovr)
                        throw Runtime_Error(xtc_format,
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(lhs.is_integer()) {
                      // __addm integer
                      V_integer& val = lhs.open_integer();
                      V_integer result = addm(val, irhs);
                      val = result;
                    }
                    else
                      throw Runtime_Error(xtc_format,
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(lhs.is_integer()) {
                      // __subm integer
                      V_integer& val = lhs.open_integer();
                      V_integer result = subm(val, irhs);
                      val = result;
                    }
                    else
                      throw Runtime_Error(xtc_format,
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(lhs.is_integer()) {
                      // __mulm integer
                      V_integer& val = lhs.open_integer();
                      V_integer result = mulm(val, irhs);
                      val = result;
                    }
                    else
                      throw Runtime_Error(xtc_format,
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(lhs.is_integer()) {
                      ...
                      // string <<< ; bytewise, fixed-width
                      V_string& val = lhs.open_string();
                      size_t n = clamp_cast<size_t>(irhs, 0, val.ssize());
                      val.erase(0, n);
                      val.append(n, '\0');
                    }
                    else if(lhs.is_array()) {
                ...
                +[](Executive_Context& ctx, const AVM_Rod::Header* head)
                  __attribute__((__hot__, __flatten__))
                  {
                    ...
                    if(lhs.is_integer()) {
                      ...
                      // string <<< ; bytewise, variable-width
                      V_string& val = lhs.open_string();
                      size_t n = clamp_cast<size_t>(irhs, 0, PTRDIFF_MAX);
                      val.append(n, '\0');
                    }
                    else if(lhs.is_array()) {
                      // array <<< ; element-wise, variable-width
