...
class Value
  ...
  public:
    ...
    template<typename AltT, ROCKET_ENABLE_IF(std::is_convertible<AltT, Variant>::value)> Value(AltT &&alt)
      : m_stor(std::forward<AltT>(alt))
      {
      }
    template<typename AltT, ROCKET_ENABLE_IF(std::is_convertible<AltT, Variant>::value)> Value & operator=(AltT &&alt)
      {
