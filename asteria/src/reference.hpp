...
extern Reference::Type get_reference_type(Sparg<const Reference> reference_opt) noexcept;

extern void dump_reference(std::ostream &os, Sparg<const Reference> reference_opt, unsigned indent_next = 0, unsigned indent_increment = 2);
extern std::ostream & operator<<(std::ostream &os, const Sptr_fmt<Reference> &reference_fmt);

extern void copy_reference(Xptr<Reference> &reference_out, Sparg<const Reference> source_opt);
extern void move_reference(Xptr<Reference> &reference_out, Xptr<Reference> &&source_opt);

extern Sptr<const Variable> read_reference_opt(Sparg<const Reference> reference_opt);
extern std::reference_wrapper<Xptr<Variable>> drill_reference(Sparg<const Reference> reference_opt);
...
extern void extract_variable_from_reference(Xptr<Variable> &variable_out, Sparg<Recycler> recycler, Xptr<Reference> &&reference_opt);

// If the reference is a temporary value, convert it to an unnamed local variable, allowing further modification to it.
extern void materialize_reference(Xptr<Reference> &reference_inout_opt, Sparg<Recycler> recycler, bool constant);
