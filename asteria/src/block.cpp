...
void bind_block_in_place(Vp<Block> &bound_block_out, Sp_cref<Scope> scope_inout, Sp_cref<const Block> block_opt){
...
void fly_over_block_in_place(Sp_cref<Scope> scope_inout, Sp_cref<const Block> block_opt){
	if(block_opt == nullptr){
		// Nothing to do.
		return;
	}
...
Statement::Execution_result execute_block_in_place(Vp<Reference> &ref_out, Sp_cref<Scope> scope_inout, Sp_cref<Recycler> recycler_out, Sp_cref<const Block> block_opt){
	if(block_opt == nullptr){
		// Nothing to do.
...
void bind_block(Vp<Block> &bound_block_out, Sp_cref<const Block> block_opt, Sp_cref<const Scope> scope){
	if(block_opt == nullptr){
		// Return a null block.
		bound_block_out.reset();
		return;
...
Statement::Execution_result execute_block(Vp<Reference> &ref_out, Sp_cref<Recycler> recycler_out, Sp_cref<const Block> block_opt, Sp_cref<const Scope> scope){
	if(block_opt == nullptr){
		// Nothing to do.
		move_reference(ref_out, nullptr);
		return Statement::execution_result_next;
