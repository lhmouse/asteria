...
namespace Asteria {
    ...
    cow_vector<AIR_Node>& do_generate_expression_partial(cow_vector<AIR_Node>& code,
                                                         const Compiler_Options& options, TCO_Aware tco_aware, const Analytic_Context& ctx,
    ...
    cow_vector<AIR_Node> do_generate_expression_partial(const Compiler_Options& options, TCO_Aware tco_aware, const Analytic_Context& ctx,
                                                        const cow_vector<Xprunit>& units)
      {
        cow_vector<AIR_Node> code;
        do_generate_expression_partial(code, options, tco_aware, ctx, units);
        return code;
      }
    ...
    cow_vector<AIR_Node>& do_generate_statement_list(cow_vector<AIR_Node>& code, cow_vector<phsh_string>* names_opt, Analytic_Context& ctx,
                                                     const Compiler_Options& options, TCO_Aware tco_aware, const cow_vector<Statement>& stmts)
      {
        size_t epos = stmts.size() - 1;
        if(epos != SIZE_MAX) {
          // Statements other than the last one cannot be the end of function.
          for(size_t i = 0; i != epos; ++i) {
            stmts[i].generate_code(code, names_opt, ctx, options, stmts[i+1].is_empty_return() ? tco_aware_nullify : tco_aware_none);
          }
          // The last statement may be TCO'd.
          stmts[epos].generate_code(code, names_opt, ctx, options, tco_aware);
        }
        return code;
      }
    cow_vector<AIR_Node> do_generate_statement_list(cow_vector<phsh_string>* names_opt, Analytic_Context& ctx,
      ...
      {
        cow_vector<AIR_Node> code;
        do_generate_statement_list(code, names_opt, ctx, options, tco_aware, stmts);
        return code;
      }

    cow_vector<AIR_Node> do_generate_block(const Compiler_Options& options, TCO_Aware tco_aware, const Analytic_Context& ctx,
                                           const cow_vector<Statement>& stmts)
      {
        ...
        auto code = do_generate_statement_list(nullptr, ctx_stmts, options, tco_aware, stmts);
        return code;
      }
    ...
    case index_expression:
      {
        ...
        if(!altr.expr.empty()) {
          ...
          do_generate_expression_partial(code, options, tco_aware, ctx, altr.expr);
        }
        return code;
      }
    case index_block:
      {
        ...
        // This can be TCO'd.
        auto code_body = do_generate_block(options, tco_aware, ctx, altr.body);
        // Encode arguments.
    ...
    case index_if:
      {
        ...
        do_generate_expression_partial(code, options, tco_aware_none, ctx, altr.cond);
        // The result will have been pushed onto the top of the stack.
        // Generate code for both branches.
        // Both can be TCO'd.
        auto code_true = do_generate_block(options, tco_aware, ctx, altr.branch_true);
        auto code_false = do_generate_block(options, tco_aware, ctx, altr.branch_false);
        // Encode arguments.
    ...
    case index_switch:
      {
        ...
        do_generate_expression_partial(code, options, tco_aware_none, ctx, altr.ctrl);
    ...
    case index_do_while:
      {
        ...
        auto code_body = do_generate_block(options, tco_aware_none, ctx, altr.body);
        // Generate code for the condition.
        ROCKET_ASSERT(!altr.cond.empty());
        auto code_cond = do_generate_expression_partial(options, tco_aware_none, ctx, altr.cond);
        // Encode arguments.
    ...
    case index_for:
      {
        ...
        auto code_init = do_generate_statement_list(nullptr, ctx_for, options, tco_aware_none, altr.init);
        auto code_cond = do_generate_expression_partial(options, tco_aware_none, ctx_for, altr.cond);
        auto code_step = do_generate_expression_partial(options, tco_aware_none, ctx_for, altr.step);
        // Generate code for the body.
        // Loop statements cannot be TCO'd.
        auto code_body = do_generate_block(options, tco_aware_none, ctx_for, altr.body);
        // Encode arguments.
    ...
    case index_try:
      {
        ...
        auto code_try = do_generate_block(options, tco_aware_none, ctx, altr.body_try);
    ...
    case index_return:
      {
        ...
        if(altr.expr.empty()) {
          ...
          do_generate_expression_partial(code, options, tco_aware_by_ref, ctx, altr.expr);
          // Forward the result as is.
          AIR_Node::S_simple_status xnode_ret = { air_status_return };
          code.emplace_back(rocket::move(xnode_ret));
        }
          ...
          do_generate_expression_partial(code, options, tco_aware_by_val, ctx, altr.expr);
