...
Statement bind_statement_partial(Analytic_context &ctx_inout, const Statement &stmt)
  {
    ...
    case Statement::index_expression:
      {
        const auto &alt = stmt.as<Statement::S_expression>();
    ...
    case Statement::index_var_def:
      {
        const auto &alt = stmt.as<Statement::S_var_def>();
    ...
    case Statement::index_func_def:
      {
        const auto &alt = stmt.as<Statement::S_func_def>();
        // Create a dummy reference for further name lookups.
    ...
    case Statement::index_if:
      {
        const auto &alt = stmt.as<Statement::S_if>();
    ...
    case Statement::index_switch:
      {
        const auto &alt = stmt.as<Statement::S_switch>();
    ...
    case Statement::index_do_while:
      {
        const auto &alt = stmt.as<Statement::S_do_while>();
    ...
    case Statement::index_while:
      {
        const auto &alt = stmt.as<Statement::S_while>();
    ...
    case Statement::index_for:
      {
        const auto &alt = stmt.as<Statement::S_for>();
    ...
    case Statement::index_for_each:
      {
        const auto &alt = stmt.as<Statement::S_for_each>();
    ...
    case Statement::index_try:
      {
        const auto &alt = stmt.as<Statement::S_try>();
    ...
    case Statement::index_break:
      {
        const auto &alt = stmt.as<Statement::S_break>();
        // Copy it as-is.
    ...
    case Statement::index_continue:
      {
        const auto &alt = stmt.as<Statement::S_continue>();
        // Copy it as-is.
    ...
    case Statement::index_throw:
      {
        const auto &alt = stmt.as<Statement::S_throw>();
    ...
    case Statement::index_return:
      {
        const auto &alt = stmt.as<Statement::S_return>();
    ...
    case Statement::index_export:
      {
        const auto &alt = stmt.as<Statement::S_export>();
        // Copy it as-is.
    ...
    case Statement::index_import:
      {
        const auto &alt = stmt.as<Statement::S_import>();
        // Copy it as-is.
