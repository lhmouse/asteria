...
namespace {
  ...
  inline void do_tell_source_location(String &file_out, Uint64 &line_out, const Token_stream &toks_io)
    {
      const auto qtok = toks_io.peek_opt();
      if(!qtok) {
        return;
  ...
  bool do_accept_import_directive(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // import-directive ::=
  ...
  extern bool do_accept_statement_as_block(Vector<Statement> &stmts_out, Token_stream &toks_io);
  extern bool do_accept_statement(Vector<Statement> &stmts_out, Token_stream &toks_io);
  extern bool do_accept_expression(Vector<Xpnode> &nodes_out, Token_stream &toks_io);

  bool do_accept_block_statement_list(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // block ::=
      //   "{" statement-list-opt "}"
      // statement-list-opt ::=
      ...
      for(;;) {
        bool stmt_got = do_accept_statement(stmts_out, toks_io);
        if(stmt_got == false) {
          break;
        }
      }
  ...
  bool do_accept_block_statement(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      Vector<Statement> stmts;
      if(do_accept_block_statement_list(stmts, toks_io) == false) {
        return false;
      }
  ...
  bool do_accept_null_statement(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // null-statement ::=
      //   ";"
      if(do_match_punctuator(toks_io, Token::punctuator_semicol) == false) {
  ...
  bool do_accept_closure_function(Vector<Xpnode> &nodes_out, Token_stream &toks_io)
    {
      ...
      if(do_accept_statement_as_block(body, toks_io) == false) {
  ...
  bool do_accept_nested_expression(Vector<Xpnode> &nodes_out, Token_stream &toks_io)
    {
      ...
      if(do_accept_expression(nodes_out, toks_io) == false) {
        throw do_make_parser_result(toks_io, Parser_result::error_expression_expected);
      }
      if(do_match_punctuator(toks_io, Token::punctuator_parenth_cl) == false) {
        throw do_make_parser_result(toks_io, Parser_result::error_close_parenthesis_expected);
  ...
  bool do_accept_expression(Vector<Xpnode> &nodes_out, Token_stream &toks_io)
    {
      // TODO
      return do_accept_infix_element(nodes_out, toks_io);
    }

  bool do_accept_variable_definition(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // variable-definition ::=
  ...
  bool do_accept_immutable_variable_definition(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
  ...
  bool do_accept_function_definition(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // Copy these parameters before reading from the stream which is destructive.
      String file;
      Uint64 line = 0;
  ...
  bool do_accept_expression_statement(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
  ...
  bool do_accept_if_statement(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // if-statement ::=
      ...
      if(do_accept_statement_as_block(branch_true, toks_io) == false) {
        throw do_make_parser_result(toks_io, Parser_result::error_statement_expected);
      }
      Vector<Statement> branch_false;
      if(do_match_keyword(toks_io, Token::keyword_else)) {
        if(do_accept_statement_as_block(branch_false, toks_io) == false) {
          throw do_make_parser_result(toks_io, Parser_result::error_statement_expected);
        }
      }
  ...
  bool do_accept_switch_statement(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // switch-statement ::=
      ...
      for(;;) {
        ...
        Vector<Statement> stmts;
        for(;;) {
          bool stmt_got = do_accept_statement(stmts, toks_io);
          if(stmt_got == false) {
            break;
          }
        }
  ...
  bool do_accept_do_while_statement(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // do-while-statement ::=
  ...
  bool do_accept_while_statement(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // while-statement ::=
  ...
  bool do_accept_for_statement(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // for-statement ::=
      ...
      if(do_match_keyword(toks_io, Token::keyword_each)) {
        ...
        bool init_got = do_accept_variable_definition(init, toks_io) ||
                        do_accept_null_statement(init, toks_io) ||
                        do_accept_expression_statement(init, toks_io);
        if(init_got == false) {
  ...
  bool do_accept_break_statement(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // break-statement ::=
  ...
  bool do_accept_continue_statement(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // continue-statement ::=
      //   "continue" ( "while" | "for" ) ";"
  ...
  bool do_accept_throw_statement(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // throw-statement ::=
      //   "throw" expression ";"
  ...
  bool do_accept_return_statement(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // return-statement ::=
  ...
  bool do_accept_try_statement(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // try-statement ::=
      ...
      if(do_accept_statement_as_block(body_try, toks_io) == false) {
        throw do_make_parser_result(toks_io, Parser_result::error_statement_expected);
      }
      ...
      if(do_accept_statement_as_block(body_catch, toks_io) == false) {
        throw do_make_parser_result(toks_io, Parser_result::error_statement_expected);
      }
  ...
  bool do_accept_nonblock_statement(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      ASTERIA_DEBUG_LOG("Looking for a nonblock statement: ", toks_io.empty() ? String::shallow("<no token>") : ASTERIA_FORMAT_STRING(*(toks_io.peek_opt())));
      // nonblock-statement ::=
      //   null-statement |
      ...
      //   try-statement
      return do_accept_null_statement(stmts_out, toks_io) ||
             do_accept_variable_definition(stmts_out, toks_io) ||
             do_accept_immutable_variable_definition(stmts_out, toks_io) ||
             do_accept_function_definition(stmts_out, toks_io) ||
             do_accept_expression_statement(stmts_out, toks_io) ||
             do_accept_if_statement(stmts_out, toks_io) ||
             do_accept_switch_statement(stmts_out, toks_io) ||
             do_accept_do_while_statement(stmts_out, toks_io) ||
             do_accept_while_statement(stmts_out, toks_io) ||
             do_accept_for_statement(stmts_out, toks_io) ||
             do_accept_break_statement(stmts_out, toks_io) ||
             do_accept_continue_statement(stmts_out, toks_io) ||
             do_accept_throw_statement(stmts_out, toks_io) ||
             do_accept_return_statement(stmts_out, toks_io) ||
             do_accept_try_statement(stmts_out, toks_io);
    }

  bool do_accept_statement_as_block(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // statement ::=
      //   block-statement | nonblock-statement
      return do_accept_block_statement_list(stmts_out, toks_io) ||
             do_accept_nonblock_statement(stmts_out, toks_io);
    }

  bool do_accept_statement(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // statement ::=
      //   block-statement | nonblock-statement
      return do_accept_block_statement(stmts_out, toks_io) ||
             do_accept_nonblock_statement(stmts_out, toks_io);
    }

  bool do_accept_directive_or_statement(Vector<Statement> &stmts_out, Token_stream &toks_io)
    {
      // directive-or-statement ::=
      //   directive | statement
      // directive ::=
      //   export-directive | import-directive
      return do_accept_export_directive(stmts_out, toks_io) ||
             do_accept_import_directive(stmts_out, toks_io) ||
             do_accept_statement(stmts_out, toks_io);
    }
...
Parser_result Parser::load(Token_stream &toks_io)
  try {
    ...
    while(toks_io.empty() == false) {
      ...
      if(do_accept_directive_or_statement(stmts, toks_io) == false) {
        throw do_make_parser_result(toks_io, Parser_result::error_identifier_expected);
      }
    }
    // Accept the result.
