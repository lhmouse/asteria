...
template<typename charT, typename traitsT, typename allocatorT> class basic_cow_istringstream : public basic_istream<charT, traitsT>
  ...
  public:
    explicit basic_cow_istringstream(string_type str, size_type caret = npos, ios_base::openmode which = ios_base::in)
      : basic_istream<char_type, traits_type>(&(this->m_sb)),
        m_sb(noadl::move(str), caret, which | ios_base::in)
      {
      }
    basic_cow_istringstream()
      : basic_cow_istringstream(string_type())
      {
      }
    basic_cow_istringstream(ios_base::openmode which)
      : basic_cow_istringstream(string_type(), npos, which)
      {
      }
