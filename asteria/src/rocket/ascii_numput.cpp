...
put_BU(uint64_t value, uint32_t precision)
  noexcept
  {
    ...
    *wptr = 0;
    ...
    do_write_digits_backwards(wptr - min(precision, 64U), wptr, value, 2);
    ::memcpy(wptr - 4, "..0b", 4);
    wptr -= 2;
...
put_XU(uint64_t value, uint32_t precision)
  noexcept
  {
    ...
    *wptr = 0;
    ...
    do_write_digits_backwards(wptr - min(precision, 16U), wptr, value, 16);
    ::memcpy(wptr - 4, "..0x", 4);
    wptr -= 2;
...
put_DU(uint64_t value, uint32_t precision)
  noexcept
  {
    ...
    *wptr = 0;
    ...
    do_write_digits_backwards(wptr - min(precision, 20U), wptr, value, 10);
...
put_BI(int64_t value, uint32_t precision)
  ...
  {
    bool sign = value < 0;
    uint64_t abs_value = min(-(uint64_t) value, (uint64_t) value);

    char* wptr = ::std::end(this->m_stor) - 1;
    *wptr = 0;

    do_write_digits_backwards(wptr - min(precision, 64U), wptr, abs_value, 2);
    ::memcpy(wptr - 4, ".-0b", 4);
    wptr -= 3;
...
put_XI(int64_t value, uint32_t precision)
  noexcept
  {
    ...
    *wptr = 0;
    ...
    do_write_digits_backwards(wptr - min(precision, 16U), wptr, abs_value, 16);
    ::memcpy(wptr - 4, ".-0x", 4);
    wptr -= 3;
...
put_DI(int64_t value, uint32_t precision)
  noexcept
  {
    ...
    *wptr = 0;
    ...
    do_write_digits_backwards(wptr - min(precision, 20U), wptr, abs_value, 10);
    wptr[-1] = '-';
    wptr -= 1;
...
put_BF(float value)
  noexcept
  {
    ...
    ASTERIA_ASSERT(wptr < ::std::end(this->m_stor));
    *wptr = 0;
