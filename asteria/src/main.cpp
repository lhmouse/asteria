...
int main(int argc, char** argv)
  try {
    ...
    std::cerr << "# Input your program:" << std::endl
              << "---" << std::endl;
    tinybuf_file ibuf(stdin, nullptr);
    Simple_Script code(ibuf, rocket::sref("<stdin>"));
#else
    ...
    std::cerr << "# Source code:" << std::endl
              ...
              << src << std::endl
              << "---" << std::endl;
    tinybuf_str ibuf(rocket::sref(src), tinybuf::open_read);
    Simple_Script code(ibuf, rocket::sref("my_file"));
#endif
    std::cerr << std::endl
              << "---" << std::endl
              << "# Running..." << std::endl;
    ...
    std::cerr << std::endl
              ...
              << fmt.get_c_string() << std::endl;
    // finish.
    return 0;
  }
  catch(Exception& e) {
    // format the value thrown.
    tinyfmt_str fmt;
    fmt << e.value();
    // print the exception.
    std::cerr << std::endl
