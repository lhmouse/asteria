...
D_integer std_string_compare(const D_string &text_one, const D_string &text_two)
  {
        ...
        [](const Cow_Vector<Value> & /*opaque*/, const Global_Context & /*global*/, Cow_Vector<Reference> &&args) -> Reference
          {
            ...
            D_string text_one;
            D_string text_two;
            if(reader.start().req(text_one).req(text_two)) {
              // Parse overloads.
              Argument_Reader::State rstate;
              if(reader.save_state(rstate).finish()) {
                // Call the binding function.
                auto cmp = std_string_compare(text_one, text_two);
                // Forward the result.
                Reference_Root::S_temporary ref_c = { cmp };
                return rocket::move(ref_c);
              }
              D_integer length = INT64_MAX;
              if(reader.load_state(rstate).opt(length).finish()) {
                // Call the binding function.
    ...
    ro.try_emplace(rocket::sref("starts_with"),
      D_function(make_simple_binding(
        ...
        rocket::sref("`std.string.starts_with(text, prefix)`"
                     "\n  * Checks whether `prefix` is a prefix of `text`. An empty string "
                     "\n    is considered to be a prefix of any string.                    "
    ...
    ro.try_emplace(rocket::sref("ends_with"),
      D_function(make_simple_binding(
        ...
        rocket::sref("`std.string.ends_with(text, suffix)`"
                     "\n  * Checks whether `suffix` is a suffix of `text`. An empty string "
                     "\n    is considered to be a suffix of any string.                    "
