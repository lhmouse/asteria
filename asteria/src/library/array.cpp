...
do_merge_blocks(V_array& output, bool unique, V_array& input, xComparator&& compare,
                ptrdiff_t bsize)
  {
    ASTERIA_ASSERT(output.size() >= input.size());
    ASTERIA_ASSERT(bsize > 0);
    ASTERIA_ASSERT(input.size() >= (size_t) bsize);
    ...
    while(ein - bin > bsize) {
      ...
      tb ^= 1;
      ASTERIA_ASSERT(bpos[tb] != epos[tb]);
...
std_array_shuffle(Global_Context& global, V_array data, optV_integer seed)
  {
    ...
    for(size_t k = 0;  k != data.size();  ++k) {
      ...
      size_t r = probe_origin(data.size(), static_cast<size_t>(lcg >> 16));
      if(r != k)
        swap(bptr[r], bptr[k]);
    }
    return data;
...
std_array_rotate(V_array data, V_integer shift)
  {
    ...
    rotate(data.mut_data(), 0, static_cast<size_t>(seek), data.size());
    return data;
  }
...
std_array_copy_keys(V_object source)
  {
    ...
    for_each(source, [&](const auto& p) { data.emplace_back(V_string(p.first));  });
    return data;
  }
...
std_array_copy_values(V_object source)
  {
    V_array data;
    data.reserve(source.size());
    for_each(source, [&](const auto& p) { data.emplace_back(p.second);  });
    return data;
  }
