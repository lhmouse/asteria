...
V_string&
do_append_exponent(V_string& text, ascii_numput& nump, char delim, int exp)
  {
    // Write the delimiter.
...
std_numeric_ldexp(V_real frac, V_integer exp)
  {
    return ::std::ldexp(frac, clamp_cast<int>(exp, INT_MIN, INT_MAX));
  }
...
std_numeric_pack_i16be(V_integer value)
  {
    char piece[2];
    store_be(piece, static_cast<uint16_t>(value));
...
std_numeric_pack_i16be(V_array values)
  {
    ...
    for(const auto& r : values) {
      char piece[2];
      store_be(piece, static_cast<uint16_t>(r.as_integer()));
...
std_numeric_unpack_i16be(V_string text)
  {
    ...
    for(size_t off = 0;  off != text.size();  off += 2) {
      uint16_t value = load_be<uint16_t>(text.data() + off);
...
std_numeric_pack_i16le(V_integer value)
  {
    char piece[2];
    store_le(piece, static_cast<uint16_t>(value));
    return V_string(piece, sizeof(piece));
  }
...
std_numeric_pack_i16le(V_array values)
  {
    ...
    for(const auto& r : values) {
      char piece[2];
      store_le(piece, static_cast<uint16_t>(r.as_integer()));
      text.append(piece, sizeof(piece));
    }
    return text;
  }
...
std_numeric_unpack_i16le(V_string text)
  {
    ...
    values.reserve(text.size() / 2);
    for(size_t off = 0;  off != text.size();  off += 2) {
      uint16_t value = load_le<uint16_t>(text.data() + off);
      values.emplace_back(static_cast<int16_t>(value));
    }
    return values;
  }
...
std_numeric_pack_i32be(V_integer value)
  {
    char piece[4];
    store_be(piece, static_cast<uint32_t>(value));
    return V_string(piece, sizeof(piece));
  }
...
std_numeric_pack_i32be(V_array values)
  {
    ...
    for(const auto& r : values) {
      char piece[4];
      store_be(piece, static_cast<uint32_t>(r.as_integer()));
      text.append(piece, sizeof(piece));
    }
    return text;
  }
...
std_numeric_unpack_i32be(V_string text)
  {
    ...
    for(size_t off = 0;  off != text.size();  off += 4) {
      uint32_t value = load_be<uint32_t>(text.data() + off);
...
std_numeric_pack_i32le(V_integer value)
  {
    char piece[4];
    store_le(piece, static_cast<uint32_t>(value));
    return V_string(piece, sizeof(piece));
  }
...
std_numeric_pack_i32le(V_array values)
  {
    ...
    for(const auto& r : values) {
      char piece[4];
      store_le(piece, static_cast<uint32_t>(r.as_integer()));
      text.append(piece, sizeof(piece));
    }
    return text;
  }
...
std_numeric_unpack_i32le(V_string text)
  {
    ...
    values.reserve(text.size() / 4);
    for(size_t off = 0;  off != text.size();  off += 4) {
      uint32_t value = load_le<uint32_t>(text.data() + off);
      values.emplace_back(static_cast<int32_t>(value));
    }
    return values;
  }
...
std_numeric_pack_i64be(V_integer value)
  {
    char piece[8];
    store_be(piece, static_cast<uint64_t>(value));
    return V_string(piece, sizeof(piece));
  }
...
std_numeric_pack_i64be(V_array values)
  {
    ...
    for(const auto& r : values) {
      char piece[8];
      store_be(piece, static_cast<uint64_t>(r.as_integer()));
      text.append(piece, sizeof(piece));
    }
    return text;
  }
...
std_numeric_unpack_i64be(V_string text)
  {
    ...
    for(size_t off = 0;  off != text.size();  off += 8) {
      uint64_t value = load_be<uint64_t>(text.data() + off);
...
std_numeric_pack_i64le(V_integer value)
  {
    char piece[8];
    store_le(piece, static_cast<uint64_t>(value));
    return V_string(piece, sizeof(piece));
  }
...
std_numeric_pack_i64le(V_array values)
  {
    ...
    for(const auto& r : values) {
      char piece[8];
      store_le(piece, static_cast<uint64_t>(r.as_integer()));
      text.append(piece, sizeof(piece));
    }
    return text;
  }
...
std_numeric_unpack_i64le(V_string text)
  {
    ...
    values.reserve(text.size() / 8);
    for(size_t off = 0;  off != text.size();  off += 8) {
      uint64_t value = load_le<uint64_t>(text.data() + off);
      values.emplace_back(static_cast<int64_t>(value));
    }
    return values;
  }
...
std_numeric_pack_f32be(V_real value)
  {
    char piece[4];
    store_be(piece, static_cast<float>(value));
    return V_string(piece, sizeof(piece));
  }
...
std_numeric_pack_f32be(V_array values)
  {
    ...
    for(const auto& r : values) {
      char piece[4];
      store_be(piece, static_cast<float>(r.as_real()));
      text.append(piece, sizeof(piece));
    }
    return text;
  }
...
std_numeric_unpack_f32be(V_string text)
  {
    ...
    values.reserve(text.size() / 4);
    for(size_t off = 0;  off != text.size();  off += 4) {
      float value = load_be<float>(text.data() + off);
...
std_numeric_pack_f32le(V_real value)
  {
    char piece[4];
    store_le(piece, static_cast<float>(value));
    return V_string(piece, sizeof(piece));
  }
...
std_numeric_pack_f32le(V_array values)
  {
    ...
    for(const auto& r : values) {
      char piece[4];
      store_le(piece, static_cast<float>(r.as_real()));
      text.append(piece, sizeof(piece));
    }
    return text;
  }
...
std_numeric_unpack_f32le(V_string text)
  {
    ...
    values.reserve(text.size() / 4);
    for(size_t off = 0;  off != text.size();  off += 4) {
      float value = load_le<float>(text.data() + off);
      values.emplace_back(static_cast<double>(value));
    }
    return values;
  }
...
std_numeric_pack_f64be(V_real value)
  {
    char piece[8];
    store_be(piece, static_cast<double>(value));
    return V_string(piece, sizeof(piece));
  }
...
std_numeric_pack_f64be(V_array values)
  {
    ...
    for(const auto& r : values) {
      char piece[8];
      store_be(piece, static_cast<double>(r.as_real()));
      text.append(piece, sizeof(piece));
    }
    return text;
  }
...
std_numeric_unpack_f64be(V_string text)
  {
    ...
    values.reserve(text.size() / 8);
    for(size_t off = 0;  off != text.size();  off += 8) {
      double value = load_be<double>(text.data() + off);
      values.emplace_back(static_cast<double>(value));
    }
    return values;
  }
...
std_numeric_pack_f64le(V_real value)
  {
    char piece[8];
    store_le(piece, static_cast<double>(value));
    return V_string(piece, sizeof(piece));
  }
...
std_numeric_pack_f64le(V_array values)
  {
    ...
    for(const auto& r : values) {
      char piece[8];
      store_le(piece, static_cast<double>(r.as_real()));
      text.append(piece, sizeof(piece));
    }
    return text;
  }
...
std_numeric_unpack_f64le(V_string text)
  {
    ...
    values.reserve(text.size() / 8);
    for(size_t off = 0;  off != text.size();  off += 8) {
      double value = load_le<double>(text.data() + off);
      values.emplace_back(static_cast<double>(value));
    }
    return values;
  }
