...
do_hash_file(const V_string& path)
  {
    ...
    unique_posix_fd fd(::open(path.safe_c_str(), O_RDONLY | O_NOCTTY));
    if(!fd)
      ASTERIA_THROW((
...
std_checksum_CRC32_private()
  {
    return make_refcnt<CRC32_Hasher>();
  }
...
std_checksum_Adler32_private()
  {
    return make_refcnt<Adler32_Hasher>();
  }
...
std_checksum_FNV1a32_private()
  {
    return make_refcnt<FNV1a32_Hasher>();
  }
...
std_checksum_SHA1_private()
  {
    return make_refcnt<SHA1_Hasher>();
  }
...
std_checksum_SHA224_private()
  {
    return make_refcnt<SHA224_Hasher>();
  }
...
std_checksum_SHA256_private()
  {
    return make_refcnt<SHA256_Hasher>();
  }
...
std_checksum_SHA384_private()
  {
    return make_refcnt<SHA384_Hasher>();
  }
...
std_checksum_SHA512_private()
  {
    return make_refcnt<SHA512_Hasher>();
  }
