    ...
    update(V_string& out, const void* data, size_t size)
      {
        ...
        for(;;) {
          ::uInt in = clamp_cast<::uInt>(eptr - bptr, 0, INT_MAX);
...
do_level(const optV_integer& level)
  {
    ...
    return clamp_cast<int>(*level, Z_NO_COMPRESSION, Z_BEST_COMPRESSION);
  }
...
std_zlib_Deflator_private(V_string format, optV_integer level)
  {
    return make_refcnt<Deflator>(do_wbits(format), do_level(level));
  }
...
std_zlib_Inflator_private(V_string format)
  {
    return make_refcnt<Inflator>(do_wbits(format));
  }
