...
std_system_pipe(V_string cmd, optV_array argv, optV_array envp, optV_string input)
  {
    ...
    unique_posix_fd out_r, out_w, in_r, in_w;
    int pipe_fds[2];
...
std_system_daemonize()
  {
    ...
    // processes later.
    unique_posix_fd tfd(::socket(AF_UNIX, SOCK_STREAM, 0));
    if(tfd == -1)
      ASTERIA_THROW((
...
std_system_load_conf(V_string path)
  {
    unique_ptr<::FILE, int (::FILE*)> fp(::fopen(path.safe_c_str(), "rb"),::fclose);
    if(!fp)
      ASTERIA_THROW((
    ...
    auto do_load_next = [&]
      {
        ...
        else if(ASTERIA_UNEXPECT(ch > 0x7F)) {
          // Parse a multibyte Unicode character.
          uint32_t u8len = lzcnt32((static_cast<uint32_t>(ch) << 24) ^ UINT32_MAX);
          ch &= (1 << (7 - u8len)) - 1;
        ...
        if(ch == '\n') {
          ...
          next_col += max(0, ::wcwidth(static_cast<wchar_t>(ch)));
      };
