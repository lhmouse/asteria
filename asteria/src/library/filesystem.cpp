        ...
        case rm_disp_expand:
          {
            ...
            unique_posix_dir dp(::opendir(elem.path.c_str()));
            if(!dp)
              ASTERIA_THROW((
...
std_filesystem_list(V_string path)
  {
    ...
    unique_posix_dir dp(::opendir(path.safe_c_str()));
    if(!dp) {
      if(errno == ENOENT)
        return nullopt;
...
std_filesystem_read(V_string path, optV_integer offset, optV_integer limit)
  {
    ...
    for(;;) {
      ...
      nbatch = min(nbatch * 2, clamp_cast<size_t>(rlimit, 0, INT_MAX));
...
std_filesystem_copy(V_string path_new, V_string path_old)
  {
    // Open the old file.
    unique_posix_fd fd_old(::open(path_old.safe_c_str(), O_RDONLY | O_NOCTTY));
    if(!fd_old)
      ASTERIA_THROW((
    ...
    // write-only.
    unique_posix_fd fd_new(::open(path_new.safe_c_str(),
