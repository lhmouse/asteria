...
clobbers_alt_stack()
  const noexcept
  {
    switch(static_cast<Index>(this->m_stor.index()))
      ...
      case index_branch:
        return any_of(this->m_stor.as<S_branch>().branches,
            [](const branch& br) {
              return any_of(br.units,
      ...
      case index_function_call:
        {
          ...
          if((opts.optimization_level >= 1)
             && none_of(altr.args,  // read: none of `args` clobbers `alt_stack`
                    [](const argument& arg) {
                      return any_of(arg.units,
                          [](const Expression_Unit& unit) { return unit.clobbers_alt_stack();  });
                    })) {
      ...
      case index_operator_rpn:
        {
          ...
          if(opts.optimization_level >= 1) {
            ...
            if(qrhs) {
              ...
              if(qrhs->is_null() && is_any_of(altr.xop, { xop_cmp_eq, xop_cmp_ne })) {
