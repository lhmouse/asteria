...
class Text_Reader
  ...
  public:
    Text_Reader(const Compiler_Options& opts,
      ...
      :
        m_tab_stop(clamp_cast<uint8_t>(opts.tab_stop, 1, 8)),
    ...
    consume(size_t nadd)
      noexcept
      {
        ...
        while(this->m_off != end_off) {
          char32_t cp;
          ASTERIA_ASSERT(utf8_decode(cp, this->m_str, this->m_off));
          ASTERIA_ASSERT(this->m_off <= end_off);
          if(cp == '\t')
            this->m_column += this->m_tab_stop - (this->m_column - 1) % this->m_tab_stop;
          else
            this->m_column += max(0, ::wcwidth(static_cast<wchar_t>(cp)));
        }
      }
...
do_may_infix_operators_follow(cow_vector<Token>& tokens)
  {
    ...
    if(p.is_punctuator())
      return is_any_of(p.as_punctuator(),
...
do_accept_numeric_literal(cow_vector<Token>& tokens, Text_Reader& reader)
  {
    ...
    switch(reader.peek(tlen))
      ...
      case '0':
        ...
        if(is_any_of(reader.peek(tlen) | 0x20, { 'b', 'x' })) {
          tstr += reader.peek(tlen);
          tlen += 1;
...
#ifdef ASTERIA_DEBUG
    ASTERIA_ASSERT(::std::is_sorted(table, table + N, comp));
#endif
...
constexpr s_keywords[] =
  {
    ...
    { "__ifcomplete",  keyword_ifcomplete  },
