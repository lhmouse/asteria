...
#include "../../compiler/infix_element.hpp"
...
do_accept_prefix_operator(cow_vector<Expression_Unit>& units, Token_Stream& tstrm)
  {
    ...
    auto sloc = qtok->sloc();
    if(qtok->is_keyword()) {
      auto qconf = find(s_prefix_keyword_xop, qtok->as_keyword());
      if(!qconf)
        return false;
    ...
    if(qtok->is_punctuator()) {
      auto qconf = find(s_prefix_punctuator_xop, qtok->as_punctuator());
      if(!qconf)
        return false;
...
do_accept_prefix_binary_operator_opt(Token_Stream& tstrm)
  {
    ...
    auto sloc = qtok->sloc();
    if(qtok->is_keyword()) {
      auto qconf = find(s_prefix_keyword_binary_xop, qtok->as_keyword());
      if(!qconf)
        return nullopt;
...
do_accept_postfix_operator(cow_vector<Expression_Unit>& units, Token_Stream& tstrm)
  {
    ...
    auto sloc = qtok->sloc();
    if(qtok->is_punctuator()) {
      auto qconf = find(s_postfix_punctuator_xop, qtok->as_punctuator());
      if(!qconf)
        return false;
...
do_accept_infix_operator_general_opt(Token_Stream& tstrm)
  {
    ...
    auto sloc = qtok->sloc();
    if(qtok->is_punctuator()) {
      auto qconf = find(s_infix_punctuator_xop, qtok->as_punctuator());
      if(!qconf)
        return nullopt;
