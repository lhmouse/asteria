...
#ifndef ASTERIA_ROCKET_SHARED_FUNCTION_
#define ASTERIA_ROCKET_SHARED_FUNCTION_
...
class shared_function<xreturnT (xparamsT...)>
  ...
  public:
    ...
    constexpr
    shared_function(function_type* pfn)
      noexcept
      : m_pfn(pfn)
      { }

    shared_function&
    operator=(function_type* pfn)
      & noexcept
      {
        this->reset(pfn);
        return *this;
    ...
    template<typename xfuncT,
    ASTERIA_ENABLE_IF(is_viable<xfuncT>::value),
    ASTERIA_DISABLE_SELF(shared_function, xfuncT)>
    explicit
    ...
    shared_function(const shared_function& other)
      noexcept
      : m_pobj(other.m_pobj),
        m_pfn(other.m_pfn)
      { }
    ...
    shared_function&
    operator=(const shared_function& other)
      & noexcept
      {
    ...
    shared_function(shared_function&& other)
      noexcept
    ...
    shared_function&
    operator=(shared_function&& other)
      & noexcept
      {
  ...
  public:
    ...
    shared_function&
    reset(function_type* pfn = nullptr)
      noexcept
      {
        this->m_pobj.reset();
        this->m_pfn = pfn;
...
inline
void
swap(shared_function<xfuncT>& lhs, shared_function<xfuncT>& rhs)
  noexcept
  { lhs.swap(rhs);  }
