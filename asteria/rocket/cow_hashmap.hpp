...
template<typename keyT, typename mappedT, typename hashT, typename eqT, typename allocT> class cow_hashmap
  ...
  public:
    ...
    explicit cow_hashmap(size_type res_arg,
    ...
    template<typename inputT, ROCKET_ENABLE_IF(is_input_iterator<inputT>::value)> cow_hashmap(inputT first, inputT last,
  ...
  public:
    ...
    // N.B. The return type is a non-standard extension.
    template<typename inputT, ROCKET_ENABLE_IF(is_input_iterator<inputT>::value)> cow_hashmap& insert(inputT first, inputT last)
      {
        if(first == last)
          return *this;
    ...
    // N.B. This is a non-standard extension.
    template<typename ykeyT, typename ydefaultT> typename select_type<const mapped_type&, ydefaultT&&>::type get_or(const ykeyT& key, ydefaultT&& ydef) const
      {
        auto ptr = this->do_get_table();
        size_type tpos;
        if(!this->m_sth.index_of(tpos, key))
    ...
    // N.B. This is a non-standard extension.
    template<typename ykeyT, typename ydefaultT> typename select_type<mapped_type&&, ydefaultT&&>::type move_or(const ykeyT& key, ydefaultT&& ydef) const
      {
        auto ptr = this->do_mut_table();
        size_type tpos;
        if(!this->m_sth.index_of(tpos, key))
