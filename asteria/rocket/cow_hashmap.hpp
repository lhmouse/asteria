...
#define ASTERIA_ROCKET_COW_HASHMAP_
...
class cow_hashmap
...
#ifndef ASTERIA_ROCKET_NO_STRICT_HASH_NOEXCEPT
...
#endif  // ASTERIA_NO_STRICT_HASH_NOEXCEPT
  ...
  public:
    ...
    ASTERIA_CONSTEXPR_INLINE cow_hashmap()
    ...
    explicit constexpr cow_hashmap(const allocator_type& alloc, const hasher& hf = hasher(),
  ...
  public:
    ...
    insert(inputT first, inputT last)
      {
        ...
        size_type tpos;
        if(ASTERIA_EXPECT(dist && bkts && (dist <= cap - this->size()))) {
    ...
    try_emplace(ykeyT&& ykey, paramsT&&... params)
      {
        ...
        size_type cap = this->capacity();
        size_type tpos;
        if(ASTERIA_EXPECT(bkts && (this->size() < cap))) {
    ...
    // N.B. The return type differs from `std::unordered_map`.
    template<typename ykeyT,
    ASTERIA_DISABLE_IF(is_convertible<ykeyT, const_iterator>::value)>
    bool
    ...
    erase(const_iterator first, const_iterator last)
      {
        ASTERIA_ASSERT_MSG(first.m_cur <= last.m_cur, "invalid range");
    ...
    find_and_copy(mapped_type& result, const ykeyT& ykey)
      const
      {
        ...
        if(!this->m_sth.find(tpos, ykey))
          return false;
        result = this->do_buckets()[tpos]->second;
        return true;
      }
    ...
    find_and_erase(mapped_type& result, const ykeyT& ykey)
      {
        ...
        if(!this->m_sth.find(tpos, ykey))
          return false;
        result = move(this->do_mut_buckets()[tpos]->second);
        this->do_erase_unchecked(tpos, 1);
        return true;
      }
