...
#define ASTERIA_ROCKET_COW_VECTOR_
...
class cow_vector
  ...
  public:
    ...
    ASTERIA_ENABLE_IF(is_constructible<value_type, const firstT&, const restT&...>::value),
    ASTERIA_DISABLE_IF(is_convertible<const firstT&, allocator_type>::value)>
...
#define ASTERIA_COW_VECTOR_VALIDATE_SUBSCRIPT_(pos, op)  \
        if(!(pos op this->size()))  \
    ...
    do_clamp_subvec(size_type tpos, size_type tn)
      const
      {
        ASTERIA_COW_VECTOR_VALIDATE_SUBSCRIPT_(tpos, <=);
  ...
  public:
    ...
    at(size_type pos)
      const
      {
        ASTERIA_COW_VECTOR_VALIDATE_SUBSCRIPT_(pos, <);
    ...
    // N.B. This is a non-standard extension.
    template<typename... paramsT,
    ASTERIA_ENABLE_IF(is_constructible<value_type, const paramsT&...>::value)>
    cow_vector&
    ...
    append(inputT first, inputT last)
      {
        ...
        if(ASTERIA_EXPECT(dist && (dist == n))) {
          ...
          for(auto it = move(first);  it != last;  ++it) {
            if(ASTERIA_UNEXPECT(sth.size() >= cap)) {
    ...
    erase(const_iterator first, const_iterator last)
      {
        ASTERIA_ASSERT_MSG(first <= last, "invalid range");
