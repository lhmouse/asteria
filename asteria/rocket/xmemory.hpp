...
#ifndef ASTERIA_ROCKET_XMEMORY_
#define ASTERIA_ROCKET_XMEMORY_
...
xmemcopy(xmeminfo& info, const xmeminfo& src)
  noexcept
  {
    ASTERIA_ASSERT(info.element_size * info.count >= src.element_size * src.count);
    ASTERIA_ASSERT(info.data != src.data);
