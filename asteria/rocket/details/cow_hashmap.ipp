...
#ifndef ASTERIA_ROCKET_COW_HASHMAP_
#  error Please include <asteria/cow_hashmap.hpp> instead.
#endif
...
struct basic_storage
    ...
    public ebo_select<hashT, allocT>
  {
    ...
    adopt_value_unchecked(pointer qval)
      ...
      {
        ASTERIA_ASSERT(qval);
        ASTERIA_ASSERT_MSG(this->nref.unique(), "shared storage shall not be modified");
...
struct storage_traits
  {
    ...
    dispatch_clone(storage_type& st_new, const storage_type& st_old)
      {
        ASTERIA_ASSERT(st_new.nelem == 0);
        ASTERIA_ASSERT(st_new.nblk == st_old.nblk);
...
class storage_handle
  ...
  public:
    ...
    keyed_try_emplace(size_type& tpos, const ykeyT& ykey, paramsT&&... params)
      {
        auto qstor = this->m_qstor;
        ASTERIA_ASSERT_MSG(qstor, "no storage allocated");
        ASTERIA_ASSERT_MSG(qstor->nref.unique(), "shared storage shall not be modified");
        ASTERIA_ASSERT_MSG(qstor->nelem < this->capacity(), "no space for new elements");
    ...
    reallocate_finish(storage_handle& sth)
      {
        ...
        for(size_t k = 0;  k != qstor->bucket_count();  ++k)
          if(auto qval = qstor->bkts[k].get())
            ASTERIA_ASSERT(!sth.find(tpos, qval->first));
#endif
...
class stringified_key
  ...
  public:
    ...
    template<typename valueT,
    ASTERIA_DISABLE_IF(is_scalar<valueT>::value)>
...
class iterator
  ...
  public:
    ...
    ASTERIA_ENABLE_IF(is_convertible<ybucketT*, bucketT*>::value)>
    constexpr
  ...
  private:
    ...
    do_validate(bucketT* cur, bool deref)
      const noexcept
      {
        ASTERIA_ASSERT_MSG(this->m_begin, "iterator not initialized");
        ASTERIA_ASSERT_MSG((this->m_begin <= cur) && (cur <= this->m_end), "iterator out of range");
        ASTERIA_ASSERT_MSG(!deref || (cur < this->m_end), "past-the-end iterator not dereferenceable");
        ASTERIA_ASSERT_MSG(!deref || *cur, "iterator invalidated");
        return cur;
      }
    ...
    do_this_pos(const bucketT* begin)
      ...
      {
        ASTERIA_ASSERT_MSG(this->m_begin, "iterator not initialized");
        ASTERIA_ASSERT_MSG(this->m_begin == begin, "iterator not compatible");
    ...
    do_this_len(const iterator& other)
      ...
      {
        ASTERIA_ASSERT_MSG(this->m_begin, "iterator not initialized");
        ASTERIA_ASSERT_MSG(this->m_begin == other.m_begin, "iterator not compatible");
        ASTERIA_ASSERT_MSG(this->m_end == other.m_end, "iterator not compatible");
    ...
    do_next()
      const noexcept
      {
        ...
        auto res = *this;
        do {
          ASTERIA_ASSERT_MSG(res.m_cur != this->m_end, "past-the-end iterator not incrementable");
          res.m_cur++;
    ...
    do_prev()
      const noexcept
      {
        ...
        auto res = *this;
        do {
          ASTERIA_ASSERT_MSG(res.m_cur != this->m_begin, "beginning iterator not decrementable");
          res.m_cur--;
        } while(!*(res.m_cur));
        return res;
      }
