...
#  error Please include <asteria/cow_string.hpp> instead.
#endif
...
class iterator
  ...
  private:
    ...
    constexpr
    iterator(charT* begin, size_t ncur, size_t nend)
      noexcept
      :
  ...
  public:
    ...
    ASTERIA_ENABLE_IF(is_convertible<ycharT*, charT*>::value)>
    constexpr iterator(const iterator<stringT, ycharT>& other)
      noexcept
      :
    ...
    ASTERIA_ENABLE_IF(is_convertible<ycharT*, charT*>::value)>
    iterator&
    operator=(const iterator<stringT, ycharT>& other)
      & noexcept
      {
...
do_boyer_moore_horspool_search(textT tbegin, textT tend, pattT pbegin, pattT pend, sizeT npos)
  {
    ...
    ASTERIA_ASSERT(tend - tbegin >= pend - pbegin);
    ASTERIA_ASSERT(pend - pbegin >= 1);
