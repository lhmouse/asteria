...
#  error Please include <asteria/linear_buffer.hpp> instead.
#endif
...
class basic_storage
  ...
  public:
    ...
    reserve(size_type goff, size_type eoff, size_type nadd)
      {
        ASTERIA_ASSERT(goff <= eoff);
        ASTERIA_ASSERT(eoff <= this->m_cap);
        ...
        if(ASTERIA_EXPECT(nadd <= this->m_cap - nused)) {
        ...
        ASTERIA_ASSERT(nused + nadd <= this->m_cap);
        return nused;
      }
