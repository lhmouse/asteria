...
#ifndef ASTERIA_ROCKET_COW_VECTOR_
#  error Please include <asteria/cow_vector.hpp> instead.
#endif
...
struct basic_storage
    ...
    public allocator_wrapper_base_for<allocT>::type
  {
    ...
    emplace_back_unchecked(paramsT&&... params)
      {
        ...
        ASTERIA_ASSERT_MSG(this->nelem < this->max_nelem_for_nblk(this->nblk), "no space for new elements");
    ...
    pop_back_unchecked()
      ...
      {
        ASTERIA_ASSERT_MSG(this->nref.unique(), "shared storage shall not be modified");
        ASTERIA_ASSERT_MSG(this->nelem > 0, "no element to pop");
...
struct storage_traits
  {
    ...
    dispatch_transfer(storage_type& st_new, storage_type& st_old, size_t nskip)
      {
        ASTERIA_ASSERT(st_old.nskip <= st_old.nelem);
        ASTERIA_ASSERT(st_old.nelem == st_new.nskip);
        ASTERIA_ASSERT(st_new.nskip <= st_new.nelem);
        ASTERIA_ASSERT(st_old.nskip <= nskip);
...
class iterator
  ...
  public:
    ...
    ASTERIA_ENABLE_IF(is_convertible<yvalueT*, valueT*>::value)>
    constexpr iterator(const iterator<vectorT, yvalueT>& other)
      noexcept
      :
