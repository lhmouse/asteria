...
#ifndef ASTERIA_ROCKET_VARIANT_
#define ASTERIA_ROCKET_VARIANT_
...
class variant
  ...
  private:
    using my_storage = storage_for<altsT...>;
  ...
  public:
    ...
    ASTERIA_CONSTEXPR_INLINE variant()
    ...
    ASTERIA_ENABLE_IF_HAS_VALUE(index_of<typename decay<paramT>::type>::value)>
    ...
    // 23.7.3.3, assignment
    template<typename paramT,
    ASTERIA_ENABLE_IF_HAS_VALUE(index_of<paramT>::value)>
    variant&
    operator=(const paramT& param)
                             ...
                             is_nothrow_copy_constructible<paramT>>::value)
      {
        ...
        if(index_old == index_new) {
          ...
          ASTERIA_ASSERT(this->m_index == index_new);
        }
  ...
  public:
    ...
    index()
      const noexcept
      {
        ASTERIA_ASSERT(this->m_index < alternative_size);
        return this->m_index;
      }
    ...
    template<typename targetT,
    ASTERIA_ENABLE_IF_HAS_VALUE(index_of<targetT>::value)>
    const targetT*
    ptr()
      const noexcept
      {
