...
#ifndef ASTERIA_ROCKET_LINEAR_BUFFER_
#define ASTERIA_ROCKET_LINEAR_BUFFER_
...
class basic_linear_buffer
  ...
  public:
    ...
    basic_linear_buffer(basic_linear_buffer&& other, const allocator_type& alloc)
      ...
      :
        m_stor(alloc)
      {
        if(ASTERIA_EXPECT(this->m_stor.as_allocator() == alloc)) {
    ...
    operator=(const basic_linear_buffer& other)
      &
      {
        if(ASTERIA_EXPECT(this->m_stor.as_allocator() == other.m_stor.as_allocator())) {
          // If the allocators compare equal, they can deallocate memory allocated by each other.
          noadl::propagate_allocator_on_copy(this->m_stor.as_allocator(), other.m_stor.as_allocator());
          if(ASTERIA_EXPECT(this != &other))
    ...
    swap(basic_linear_buffer& other)
      ...
      {
        ASTERIA_ASSERT((this->m_stor.as_allocator() == other.m_stor.as_allocator()) ||
                      allocator_traits<allocator_type>::propagate_on_container_swap::value);
  ...
  public:
    ...
    discard(size_type nbump)
      noexcept
      {
        ASTERIA_ASSERT(nbump <= this->m_eoff - this->m_goff);
#ifdef ASTERIA_DEBUG
    ...
    reserve_after_end(size_type nbump)
      {
        if(ASTERIA_UNEXPECT(nbump > this->capacity_after_end())) {
    ...
    accept(size_type nbump)
      noexcept
      {
        ASTERIA_ASSERT(nbump <= this->capacity_after_end());
        this->m_eoff += nbump;
        return *this;
      }

    basic_linear_buffer&
    unaccept(size_type nbump)
      noexcept
      {
        ASTERIA_ASSERT(nbump <= this->m_eoff - this->m_goff);
        this->m_eoff -= nbump;
