...
template<typename valueT> class optional
  ...
  public:
    ...
    optional& operator=(const value_type& value) noexcept(conjunction<is_nothrow_copy_constructible<value_type>,
    ...
    optional& operator=(value_type&& value) noexcept(conjunction<is_nothrow_move_constructible<value_type>,
    ...
    template<typename yvalueT, ROCKET_ENABLE_IF(is_assignable<value_type, yvalueT&&>::value)>
                  optional& operator=(yvalueT&& yvalue) noexcept(conjunction<is_nothrow_constructible<value_type, yvalueT&&>,
    ...
    template<typename yvalueT, ROCKET_ENABLE_IF(is_assignable<value_type, const typename optional<yvalueT>::value_type&>::value)>
                  optional& operator=(const optional<yvalueT>& other) noexcept(conjunction<is_nothrow_constructible<value_type, const typename optional<yvalueT>::value_type&>,
    ...
    template<typename yvalueT, ROCKET_ENABLE_IF(is_assignable<value_type, typename optional<yvalueT>::value_type&&>::value)>
                  optional& operator=(optional<yvalueT>&& other) noexcept(conjunction<is_nothrow_constructible<value_type, typename optional<yvalueT>::value_type&&>,
  ...
  public:
    ...
    template<typename dvalueT> typename select_type<const_reference, dvalueT&&>::type value_or(dvalueT&& dvalue) const
      { return this->m_stor.empty() ? ::std::forward<dvalueT>(dvalue) : this->m_stor.front();  }

    // N.B. The return type differs from `std::variant`.
    template<typename dvalueT> typename select_type<reference, dvalueT&&>::type value_or(dvalueT&& dvalue)
      { return this->m_stor.empty() ? ::std::forward<dvalueT>(dvalue) : this->m_stor.mut_front();  }

    // N.B. This is a non-standard extension.
    template<typename dvalueT> typename select_type<value_type&&, dvalueT&&>::type move_value_or(dvalueT&& dvalue)
...
template<typename valueT> inline void swap(optional<valueT>& lhs, optional<valueT>& rhs) noexcept(noexcept(lhs.swap(rhs)))
  { lhs.swap(rhs);  }

template<typename valueT> constexpr bool operator==(const optional<valueT>& lhs, const optional<valueT>& rhs)
...
template<typename valueT> constexpr bool operator==(const optional<valueT>& lhs, const valueT& rhs)
                                       noexcept(noexcept(::std::declval<const valueT&>() == ::std::declval<const valueT&>()))
  { return !!lhs && (*lhs == rhs);  }
...
template<typename valueT> constexpr bool operator==(const valueT& lhs, const optional<valueT>& rhs)
                                       noexcept(noexcept(::std::declval<const valueT&>() == ::std::declval<const valueT&>()))
  { return !!rhs && (lhs == *rhs);  }
...
template<typename charT, typename traitsT, typename valueT> inline basic_tinyfmt<charT, traitsT>& operator<<(
                                                              basic_tinyfmt<charT, traitsT>& fmt, const optional<valueT>& rhs)
