...
#ifndef ASTERIA_ROCKET_FWD_
#define ASTERIA_ROCKET_FWD_

#if !defined ASTERIA_DEBUG && (defined _LIBCPP_DEBUG || defined _GLIBCXX_DEBUG)
#  define ASTERIA_DEBUG   1
#endif

#if !defined NDEBUG && !defined ASTERIA_DEBUG
#  define NDEBUG  1
#endif
...
// Utility macros
#define ASTERIA_CAR(x, ...)   x
#define ASTERIA_CDR(x, ...)   __VA_ARGS__
#define ASTERIA_CAT2(x,y)     x##y
#define ASTERIA_CAT3(x,y,z)   x##y##z

#define ASTERIA_STRINGIFY_NX(...)   #__VA_ARGS__
#define ASTERIA_STRINGIFY(...)      ASTERIA_STRINGIFY_NX(__VA_ARGS__)
#define ASTERIA_SOURCE_LOCATION     __FILE__ ":" ASTERIA_STRINGIFY(__LINE__)

#if defined __clang__
#  define ASTERIA_FORMAT_PRINTF(...)   __attribute__((__format__(__printf__, __VA_ARGS__)))
#else
#  define ASTERIA_FORMAT_PRINTF(...)   __attribute__((__format__(__gnu_printf__, __VA_ARGS__)))
#endif

#define ASTERIA_SELECTANY         __attribute__((__weak__))
#define ASTERIA_SECTION(...)      __attribute__((__section__(__VA_ARGS__)))
#define ASTERIA_NEVER_INLINE      __attribute__((__noinline__))
#define ASTERIA_PURE              __attribute__((__pure__))
#define ASTERIA_CONST             __attribute__((__const__))
#define ASTERIA_ARTIFICIAL        __attribute__((__artificial__, __always_inline__))
#define ASTERIA_FLATTEN           __attribute__((__flatten__))
#define ASTERIA_ALWAYS_INLINE     __attribute__((__always_inline__)) __inline__
#define ASTERIA_COLD              __attribute__((__cold__))
#define ASTERIA_HOT               __attribute__((__hot__))

#define ASTERIA_UNREACHABLE()       __builtin_unreachable()
#define ASTERIA_EXPECT(...)         __builtin_expect(!!(__VA_ARGS__), 1)
#define ASTERIA_UNEXPECT(...)       __builtin_expect(!!(__VA_ARGS__), 0)
#define ASTERIA_CONSTANT_P(...)     __builtin_constant_p(__VA_ARGS__)
#define ASTERIA_FUNCSIG             __PRETTY_FUNCTION__

#define ASTERIA_UNDEDUCED(...)    typename ::std::enable_if<true, __VA_ARGS__>::type
#define ASTERIA_VOID_T(...)       typename ::std::conditional<true, void, __VA_ARGS__>::type
#define ASTERIA_ENABLE_IF(...)    typename ::std::enable_if<+bool(__VA_ARGS__)>::type* = nullptr
#define ASTERIA_DISABLE_IF(...)   typename ::std::enable_if<!bool(__VA_ARGS__)>::type* = nullptr

#define ASTERIA_VOID_DECLTYPE(...)         ASTERIA_VOID_T(decltype(__VA_ARGS__))
#define ASTERIA_ENABLE_IF_HAS_TYPE(...)    ASTERIA_VOID_T(__VA_ARGS__)* = nullptr
#define ASTERIA_ENABLE_IF_HAS_VALUE(...)   ASTERIA_ENABLE_IF(sizeof(__VA_ARGS__) != 0)
#define ASTERIA_DISABLE_SELF(T, ...)       ASTERIA_DISABLE_IF(::std::is_base_of<T,  \
                                              typename ::std::remove_reference<__VA_ARGS__>::type  \
                                               >::value)

#define ASTERIA_CONSTEXPR_INLINE   constexpr ASTERIA_ALWAYS_INLINE  // https://gcc.gnu.org/PR109464
#define ASTERIA_SET_IF(x, ...)   ((x) && ((void) (*(x) = (__VA_ARGS__)), true))

#define ASTERIA_DEFINE_ENUM_OPERATORS(enumT)  \
  constexpr  \
  enumT  \
...
ASTERIA_ENABLE_IF(is_integral<xvT>::value && is_integral<yvT>::value)>
constexpr
...
ASTERIA_ENABLE_IF(is_trivially_copyable<valueT>::value && (sizeof(valueT) == 2))>
...
template<typename valueT,
ASTERIA_ENABLE_IF(is_trivially_copyable<valueT>::value && (sizeof(valueT) == 4))>
ASTERIA_ALWAYS_INLINE
valueT
load_be(const void* ptr)
  noexcept
...
template<typename valueT,
ASTERIA_ENABLE_IF(is_trivially_copyable<valueT>::value && (sizeof(valueT) == 8))>
ASTERIA_ALWAYS_INLINE
valueT
load_be(const void* ptr)
  noexcept
