...
int main()
  {
    ...
    ASTERIA_TEST_CHECK(qref->read().as_integer() == 41);
    qref = ctx.get_named_reference_opt(rocket::sref("data"));
    ASTERIA_TEST_CHECK(qref != nullptr);
    ASTERIA_TEST_CHECK(qref->read().as_array().size() == 4);
    ASTERIA_TEST_CHECK(qref->read().as_array().at(0).as_integer() ==  1);
    ASTERIA_TEST_CHECK(qref->read().as_array().at(1).as_integer() ==  2);
    ASTERIA_TEST_CHECK(qref->read().as_array().at(2).as_integer() ==  3);
    ASTERIA_TEST_CHECK(qref->read().as_array().at(3).as_integer() == 10);
