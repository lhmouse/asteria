...
int main()
  {
    Value value(true);
    ASTERIA_TEST_CHECK(value.dtype() == dtype_boolean);
    ...
    value = D_integer(42);
    ASTERIA_TEST_CHECK(value.dtype() == dtype_integer);
    ASTERIA_TEST_CHECK(value.check<D_integer>() == 42);

    value = D_real(1.5);
    ASTERIA_TEST_CHECK(value.dtype() == dtype_real);
    ASTERIA_TEST_CHECK(value.check<D_real>() == 1.5);

    value = D_string(rocket::sref("hello"));
    ASTERIA_TEST_CHECK(value.dtype() == dtype_string);
    ...
    ASTERIA_TEST_CHECK(value.dtype() == dtype_array);
    ...
    ASTERIA_TEST_CHECK(value.dtype() == dtype_object);
