...
int main()
  {
    ...
    auto val = ref.read();
    ASTERIA_TEST_CHECK(val.dtype() == gtype_string);
    ASTERIA_TEST_CHECK(val.check<G_string>() == "meow");
    ...
    val = ref.read();
    ASTERIA_TEST_CHECK(val.dtype() == gtype_integer);
    ASTERIA_TEST_CHECK(val.check<G_integer>() == 42);
    ASTERIA_TEST_CHECK_CATCH(ref.open() = G_boolean(true));
    val = ref2.read();
    ASTERIA_TEST_CHECK(val.dtype() == gtype_string);
    ASTERIA_TEST_CHECK(val.check<G_string>() == "meow");
    ...
    val = ref.read();
    ASTERIA_TEST_CHECK(val.dtype() == gtype_null);
    ref.open() = G_integer(36);
    ref.zoom_out();
    ref.zoom_in(Reference_Modifier::S_array_index { 0 });
    val = ref.read();
    ASTERIA_TEST_CHECK(val.dtype() == gtype_integer);
    ASTERIA_TEST_CHECK(val.check<G_integer>() == 36);
    ref.zoom_out();
    ...
    val = ref.read();
    ASTERIA_TEST_CHECK(val.dtype() == gtype_real);
    ASTERIA_TEST_CHECK(val.check<G_real>() == 10.5);
    ref.zoom_out();
    ref.zoom_out();
