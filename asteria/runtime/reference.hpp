    ...
    dereference_copy()
      {
        ...
        // Ensure the value is copied before being moved, so we will never
        // assign an element into its own container.
        Value val = this->do_dereference_readonly_slow();
        st = &(this->m_stor.emplace<St_temp>());
        st->val = move(val);
        ROCKET_ASSERT(st->subs.empty());
        return st->val;
      }
