...
int main()
  {
    ...
    ASTERIA_TEST_CHECK(read_reference(*qref).check<D_integer>() == 41);
    qref = ctx.get_named_reference_opt(String::shallow("data"));
    ASTERIA_TEST_CHECK(qref != nullptr);
    ASTERIA_TEST_CHECK(read_reference(*qref).check<D_array>().size() == 4);
    ASTERIA_TEST_CHECK(read_reference(*qref).check<D_array>().at(0).check<D_integer>() ==  1);
    ASTERIA_TEST_CHECK(read_reference(*qref).check<D_array>().at(1).check<D_integer>() ==  2);
    ASTERIA_TEST_CHECK(read_reference(*qref).check<D_array>().at(2).check<D_integer>() ==  3);
    ASTERIA_TEST_CHECK(read_reference(*qref).check<D_array>().at(3).check<D_integer>() == 10);
