...
int main()
  {
    Simple_Script code;
    code.mut_options().optimization_level = 2;
    code.reload_string(
...
///////////////////////////////////////////////////////////////////////////////
        ...
        const os_str = std.string.to_lower(std.system.get_properties().os);
        ...
        assert __addm(1, iMax) == iMin;
        assert __addm(iMin, iMin) == 0;
        assert __addm(iMax, iMax) == -2;
        assert __addm(iMax, iMin) == -1;
        assert __addm(iMin, iMax) == -1;
        assert __subm(2, 1) == 1;
        assert __subm(iMin, 1) == iMax;
        assert __subm(0, iMin) == iMin;
        assert __subm(iMin, iMax) == 1;
        assert __subm(iMax, -1) == iMin;
        assert __subm(-2, iMax) == iMax;
        assert __subm(iMax, iMin) == -1;
        assert __mulm(2, 3) == 6;
        assert __mulm(-1, -1) == 1;
        assert __mulm(iMax, 1) == iMax;
        assert __mulm(1, iMax) == iMax;
        assert __mulm(iMax, -1) == -iMax;
        assert __mulm(-1, iMax) == -iMax;
        assert __mulm(iMin, 1) == iMin;
        assert __mulm(1, iMin) == iMin;
        assert __mulm(iMin, -1) == iMin;
        assert __mulm(-1, iMin) == iMin;
        assert __mulm(iMax, 2) == iMax <<< 1;
        assert __mulm(2, iMax) == iMax <<< 1;
        assert __mulm(iMax, -2) == -(iMax <<< 1);
        assert __mulm(-2, iMax) == -(iMax <<< 1);
        assert __mulm(iMin, 2) == iMin <<< 1;
        assert __mulm(2, iMin) == iMin <<< 1;
        assert __mulm(iMin, -2) == -(iMin <<< 1);
        assert __mulm(-2, iMin) == -(iMin <<< 1);

        assert __adds(2, 1) == 3;
        assert __adds(iMax, 1) == iMax;
        assert __adds(1, iMax) == iMax;
        assert __adds(iMin, iMin) == iMin;
        assert __adds(iMax, iMax) == iMax;
        assert __adds(iMax, iMin) == -1;
        assert __adds(iMin, iMax) == -1;
        assert __subs(2, 1) == 1;
        assert __subs(iMin, 1) == iMin;
        assert __subs(0, iMin) == iMax;
        assert __subs(iMin, iMax) == iMin;
        assert __subs(iMax, -1) == iMax;
        assert __subs(-2, iMax) == iMin;
        assert __subs(iMax, iMin) == iMax;
        assert __muls(2, 3) == 6;
        assert __muls(-1, -1) == 1;
        assert __muls(iMax, 1) == iMax;
        assert __muls(1, iMax) == iMax;
        assert __muls(iMax, -1) == -iMax;
        assert __muls(-1, iMax) == -iMax;
        assert __muls(iMin, 1) == iMin;
        assert __muls(1, iMin) == iMin;
        assert __muls(iMin, -1) == iMax;
        assert __muls(-1, iMin) == iMax;
        assert __muls(iMax, 2) == iMax;
        assert __muls(2, iMax) == iMax;
        assert __muls(iMax, -2) == iMin;
        assert __muls(-2, iMax) == iMin;
        assert __muls(iMin, 2) == iMin;
        assert __muls(2, iMin) == iMin;
