...
int main(){
	...
	Statement::S_variable_definition stmt_v = { String::shallow("sum"), false, std::move(initzr) };
	...
	initzr.emplace(std::move(initzr_a));
	stmt_v = { String::shallow("i"), false, std::move(initzr) };
	...
	expr_nodes.emplace_back(std::move(expr_l));
	Expression_node::S_named_reference expr_n = { String::shallow("i") };
	...
	// >>> sum += key + value;
	expr_nodes.clear();
	expr_n = { String::shallow("value") };
	expr_nodes.emplace_back(std::move(expr_n));
	expr_n = { String::shallow("key") };
	expr_nodes.emplace_back(std::move(expr_n));
	...
	stmts_nested.emplace_back(std::move(stmt_e));
	body.emplace(std::move(stmts_nested));
	Statement::S_for_each_statement stmt_fe = { String::shallow("key"), String::shallow("value"), std::move(initzr), std::move(body) };
	...
	ASTERIA_TEST_CHECK(scope->get_local_reference_opt(String::shallow("i")) == nullptr);
	const auto sum_ref = scope->get_local_reference_opt(String::shallow("sum"));
