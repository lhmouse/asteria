...
int main(){
	...
	ASTERIA_TEST_CHECK(var->get<D_string>() == D_string::shallow("hello"));

	D_array array;
	set_variable(var, recycler, D_boolean(true));
	array.emplace_back(std::move(var));
	set_variable(var, recycler, D_string(D_string::shallow("world")));
	array.emplace_back(std::move(var));
	...
	ASTERIA_TEST_CHECK(var->get<D_array>().at(1)->get<D_string>() == D_string::shallow("world"));
	...
	ASTERIA_TEST_CHECK(var->get<D_object>().at(D_string::shallow("one"))->get<D_boolean>() == true);
	ASTERIA_TEST_CHECK(var->get<D_object>().at(D_string::shallow("two"))->get<D_string>() == D_string::shallow("world"));
	...
	set_variable(var, recycler, D_string(D_string::shallow("hello")));
	set_variable(cmp, recycler, D_string(D_string::shallow("world")));
	ASTERIA_TEST_CHECK(compare_variables(var, cmp) == Variable::comparison_result_less);
	swap(var, cmp);
	ASTERIA_TEST_CHECK(compare_variables(var, cmp) == Variable::comparison_result_greater);
	...
	var->get<D_array>().at(1)->set(D_string(D_string::shallow("hello")));
	ASTERIA_TEST_CHECK(compare_variables(var, cmp) == Variable::comparison_result_less);
	swap(var, cmp);
	ASTERIA_TEST_CHECK(compare_variables(var, cmp) == Variable::comparison_result_greater);
