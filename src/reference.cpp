...
std::shared_ptr<const Variable> Reference::load() const {
	...
	case type_array_element: {
		...
		if(!array){
			ASTERIA_THROW("Only arrays can be indexed by integers, while the operand has type `", get_variable_type_name(array_element.variable_opt), "`");
		}
	...
	case type_object_member: {
		...
		if(!object){
			ASTERIA_THROW("Only objects can be indexed by strings, while the operand has type `", get_variable_type_name(object_member.variable_opt), "`");
		}
...
Value_ptr<Variable> &Reference::store(Value_ptr<Variable> &&new_value){
	...
	case type_direct_reference: {
		const auto &direct_reference = boost::get<Direct_reference>(m_variant);
		ASTERIA_THROW("Attempting to write to a temporary reference of type `", get_variable_type_name(direct_reference.variable_opt), "`");
		/*return ???;*/ }
	case type_array_element: {
		const auto &array_element = boost::get<Array_element>(m_variant);
		if(!array_element.variable_opt){
