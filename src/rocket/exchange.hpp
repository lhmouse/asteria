...
typename decay<elementT>::type exchange(elementT &element_old, const elementT &element_new){
	auto old = ::std::move(element_old);
	element_old = element_new;
	return old;
}
template<typename elementT>
typename decay<elementT>::type exchange(elementT &element_old, elementT &&element_new){
	auto old = ::std::move(element_old);
