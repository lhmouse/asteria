...
Value_ptr<Variable> Initializer::create_variable_opt(const std::shared_ptr<Scope> &scope) const {
	...
	case type_bracketed_init_list: {
		...
		for(const auto &child : bracketed_init_list.initializers){
			array.emplace_back(child->create_variable_opt(scope));
		}
	...
	case type_braced_init_list: {
		...
		for(const auto &pair : braced_init_list.key_values){
			object.emplace(pair.first, pair.second->create_variable_opt(scope));
		}
