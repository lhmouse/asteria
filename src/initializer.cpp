...
Xptr<Variable> initialize_variable_recursive_opt(Xptr<Variable> &variable_out, Spref<Recycler> recycler, Spref<Scope> scope, Spref<const Initializer> initializer_opt){
	...
	case Initializer::type_assignment_init: {
		...
		return copy_variable_recursive_opt(variable_out, recycler, read_reference_opt(nullptr, result)); }
	case Initializer::type_bracketed_init_list: {
		...
		for(const auto &elem : params.initializers){
			initialize_variable_recursive_opt(variable_out, recycler, scope, elem);
	...
	case Initializer::type_braced_init_list: {
		...
		for(const auto &pair : params.key_values){
			initialize_variable_recursive_opt(variable_out, recycler, scope, pair.second);
