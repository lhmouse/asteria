...
Sptr<Variable> initialize_variable_recursive(Xptr<Variable> &variable_out_opt, Spref<Recycler> recycler, Spref<Scope> scope, Spref<const Initializer> initializer_opt){
	const auto type = get_initializer_type(initializer_opt);
	switch(type){
	case Initializer::type_null: {
		return set_variable(variable_out_opt, recycler, nullptr); }
	case Initializer::type_assignment_init: {
		...
		return set_variable_using_reference(variable_out_opt, recycler, std::move(result)); }
	case Initializer::type_bracketed_init_list: {
		...
		for(const auto &elem : params.initializers){
			initialize_variable_recursive(variable_out_opt, recycler, scope, elem);
			array.emplace_back(std::move(variable_out_opt));
		}
		return set_variable(variable_out_opt, recycler, std::move(array)); }
	case Initializer::type_braced_init_list: {
		...
		for(const auto &pair : params.key_values){
			initialize_variable_recursive(variable_out_opt, recycler, scope, pair.second);
			object.emplace(pair.first, std::move(variable_out_opt));
		}
		return set_variable(variable_out_opt, recycler, std::move(object)); }
	default:
